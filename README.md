...
Portable replay for the prior adapter surface preserves the producer CPU reference, regenerates an independent verifier-local CPU reference, requires their difference to stay inside the frozen `0.001` maximum-absolute and `0.006` relative-L2 bounds, and requires the TPU output to pass independently against both. The new donated v3 surface needs its own fresh producer receipt and relocation attestation before it inherits any portable claim. Both surfaces are limited to Hq32, Hkv16, D128, contexts 128/512/1024/2048, and decode blocks 8/128/8/128; neither is full Inkling serving, a throughput promotion, or a global block-size optimum. The contract also pins the backend Python path, clean Inkling repository revision, and exact locked import-package versions that provide `sgl_jax`; compiling rejects a missing, dirty, or different backend before creating an output root, while public receipt replay remains path-relocatable.
