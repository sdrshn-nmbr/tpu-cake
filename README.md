...
The prospective Seqax BF16 numerical-v5 contract keeps the 29 pre-v4 calibration observations, excludes the 12 inspected v4 held-out observations as prior evidence, and reserves three newly derived width, batch/sequence, and depth/sequence surfaces for fresh TPU validation. The v3 run remains negative evidence: both paths independently passed their final and projection checks, but differed by one BF16 step at the normalized MLP input, showing that cross-path equality was not an RMSNorm oracle. V4 passed on its x86 TPU host but failed relocated replay because JAX CPU reductions were not byte-identical on ARM; that bundle remains unchanged portability evidence. V5 keeps the v4 RMSNorm checkpoints and adds an explicit portable CPU replay rule. The runner's saved CPU reference remains an exact receipt-bound artifact. Relocated replay regenerates the reference locally, requires the saved and fresh CPU realizations to satisfy the already-frozen cross-path numerical bounds, and requires TPU outputs to pass against both. Each path is still assessed independently against the declared RMSNorm scale, projection oracles, BF16 conversions, and final-output bounds; cross-path checkpoint distances are reporting only. The claim is bounded dual-JAX-CPU numerical agreement on the declared surface, not exact architecture-independent oracle equality. Executable discriminator replay covers RMSNorm bypass, wrong scale, corrupted statistic/inverse/normalized values, projection errors, and BF16 conversion errors. Exact StableHLO identities and relocated public receipt replay remain required. Two independent TPU7x compile captures reproduced all 40 StableHLO identities, so the runner is enabled; no v5 correctness claim exists until a fresh held-out TPU run and relocated public replay pass.
