...
A separate large-residual contract now declares a synthetic model-4096, sequence-128, one-layer workload before timing. Its BF16 residual activation is 1 MiB per data shard, 2,048 times the earlier surface and above the [Scaling Book's illustrative latency-bound communication regime](https://jax-ml.github.io/scaling-book/inference/#appendix-c-latency-bound-communications). Static analysis predicts 23,154,688 ring-equivalent bytes per device for reduce-scatter/local-residual/deferred-all-gather versus 26,300,416 for residual all-reduce, but this is only a hypothesis. Two clean TPU7x compiler captures were byte-identical and retained three native SparseCore reduce-scatters for the standard schedule. The workload is not decode or a production-model proxy; correctness qualification and independent replay must pass before its one-shot timing claim can be acquired.
