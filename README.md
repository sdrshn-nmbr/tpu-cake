...
The prospective Seqax BF16 numerical-v3 contract treats every v1/v2 seed and shape as calibration evidence and reserves three new width, batch/sequence, and depth/sequence surfaces for fresh TPU validation. It makes both the SiLU and hidden BF16 materialization boundaries explicit. Each layer returns the normalized BF16 MLP input, float32 gate projection, BF16 gate projection, SiLU, float32 up projection, BF16 up projection, hidden product, float32 down projection, and BF16 down projection. The two paths must return the same normalized input; each path must independently satisfy fixed-order float32 gate and up projection bounds, exactly cast both results to BF16, match the mathematical SiLU checkpoint within one BF16 step, form the exact BF16 hidden product, satisfy the down-projection bound, and exactly cast its down result. Cross-path checkpoint distances are reported rather than used as numerical authority. Normal and checkpoint-returning executables must each pass the same final-output policy. Executable discriminator replay includes wrong-weight and corrupted float32/BF16 gate and up checkpoints. Pinned StableHLO identities and relocated public receipt replay remain required. The v1 and v2 failures remain negative evidence. Fresh v3 StableHLO identities are currently pending, so the runner refuses execution; no v3 correctness claim exists until those identities are pinned and a new held-out TPU run and relocated public replay pass.
