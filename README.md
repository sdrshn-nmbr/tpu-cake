...
The prospective Seqax BF16 numerical-v2 contract separates every previously observed seed and shape into a calibration surface, raises the CPU-relative L2 envelope to three BF16 unit roundoffs and the row-scaled maximum envelope to eight, and reserves three new width, batch/sequence, and depth/sequence surfaces for fresh TPU validation. It retains exact typed SiLU checkpoints, exact instrumented-output parity, tighter Pallas-versus-control bounds, executable discriminator replay, pinned StableHLO identities, and relocated public receipt replay. The v1 failure remains negative evidence; v2 is not an accepted correctness claim until its fresh held-out TPU run passes unchanged.
