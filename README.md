...
The prospective Seqax BF16 numerical-v4 contract treats every v1/v2/v3 seed and shape as calibration evidence and reserves three newly derived width, batch/sequence, and depth/sequence surfaces for fresh TPU validation. The v3 run remains negative evidence: both paths independently passed their final and projection checks, but differed by one BF16 step at the normalized MLP input, showing that cross-path equality was not an RMSNorm oracle. V4 therefore returns the real pre-MLP RMSNorm input, float32 mean square, float32 inverse RMS, normalized float32 value, and normalized BF16 value before the existing gate, SiLU, up, hidden, and down checkpoints. StableHLO must bind those values to the actual RMSNorm dataflow. Each path is assessed independently against the declared scale and fixed numerical bounds; cross-path checkpoint distances are reporting only. Normal and checkpoint-returning executables must each pass the same final-output policy. Executable discriminator replay covers RMSNorm bypass, wrong scale, corrupted statistic/inverse/normalized values, projection errors, and BF16 conversion errors. Pinned StableHLO identities and relocated public receipt replay remain required. Two fresh TPU7x compile captures reproduced all v4 instrumented StableHLO identities exactly, so the runner is enabled; no v4 correctness claim exists until a new held-out TPU run and relocated public replay pass.
