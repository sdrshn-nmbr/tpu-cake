...
The prospective Seqax BF16 numerical-v3 contract treats every v1/v2 seed and shape as calibration evidence and reserves three new width, batch/sequence, and depth/sequence surfaces for fresh TPU validation. It makes both the SiLU and hidden BF16 materialization boundaries explicit, captures gate, SiLU, up, hidden, float32 down-projection, and BF16 down-projection values, and checks the hidden product and down projection independently. Normal and checkpoint-returning executables must each pass the same output policy; the BF16 checkpoint comparison permits at most one representable BF16 step, while hidden multiplication stays exact per path and the float32 down projection keeps its independent error bound. Executable discriminator replay, pinned StableHLO identities, and relocated public receipt replay remain required. The v1 and v2 failures remain negative evidence. The v3 contract now binds fresh TPU StableHLO identities, but it remains a prospective contract rather than a correctness claim until a new held-out TPU run and relocated public replay pass.
